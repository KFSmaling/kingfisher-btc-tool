...
CREATE POLICY "Eigen canvassen"
  ON canvases FOR ALL USING (user_id = auth.uid());
...
CREATE POLICY "Eigen uploads"
  ON canvas_uploads FOR ALL USING (user_id = auth.uid());
...
DROP POLICY IF EXISTS "Eigen chunks" ON document_chunks;
...
CREATE POLICY "Eigen import jobs"
  ON import_jobs FOR ALL USING (user_id = auth.uid());
...
BEGIN
  RETURN QUERY
  SELECT dc.id, dc.content, dc.metadata, dc.parent_id,
         1 - (dc.embedding <=> query_embedding) AS similarity
  FROM document_chunks dc
  WHERE dc.canvas_id = canvas_id_filter
